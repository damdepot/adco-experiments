    ...
    with open(db_config_path, "r") as f:
        try:
            ...
            for conf in configs:
                ...
                if plt == "Postgres":
                    try:
                        # Environment variables take precedence, following the
                        # standard PostgreSQL client convention (and matching
                        # src/main/python/util/Config.py). This lets the
                        # container point at a remote DB service (e.g. the `db`
                        # container in docker-compose.yml) without editing
                        # DBConfig.yaml.
                        _pgsql_user = os.environ.get("PGUSER", conf.get("user"))
                        _pgsql_password = os.environ.get("PGPASSWORD", conf.get("password"))
                        _pgsql_host = os.environ.get("PGHOST", conf.get("host"))
                        _pgsql_port = os.environ.get("PGPORT", conf.get("port"))
                        try:
                            _pgsql_port = int(_pgsql_port)
                    ...
                    try:
                        # Environment variables take precedence (MYSQL_HOST
                        # etc., matching the `db` service in docker-compose.yml).
                        _mysql_user = os.environ.get("MYSQL_USER", conf.get("user"))
                        _mysql_password = os.environ.get("MYSQL_PASSWORD", conf.get("password"))
                        _mysql_host = os.environ.get("MYSQL_HOST", conf.get("host"))
                        _mysql_port = os.environ.get("MYSQL_PORT", conf.get("port"))
                        try:
