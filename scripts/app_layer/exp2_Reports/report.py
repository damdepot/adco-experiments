...
def discover_workloads(results_dir, dbms):
    ...
    workloads = set()
    systems = ['baseline', 'adco', 'aco', 'py-dbridge']
    for sys in systems:
...
def generate_workload_report(data, raw_records, workload, dbms, systems, system_display_names, output_dir):
    ...
    # Calculate stats per transaction using median
    ...
    base_sys = systems[0]
    other_systems = systems[1:]
    speedups = {sys: [] for sys in other_systems}
    
    # Format terminal output
    lines = []
    title = f"Application Layer Performance Comparison: {workload.upper()} on {dbms}"
    lines.append(title)
    lines.append("=" * len(title))
    lines.append("")
    
    base_display = system_display_names.get(base_sys, base_sys.capitalize())
    header = f"{'Transaction':<20} | {f'{base_display} (txn/s)':<18}"
    ...
    for txn in all_txns:
        line = f"{txn:<20} | "
        base_rate = stats[base_sys].get(txn, {}).get('rate')
    ...
    if output_dir:
        ...
        md_lines = []
        md_lines.append(f"# Application Layer Performance Comparison: {workload.upper()} on {dbms}\n")
        
        # Table 1: Throughput (Transaction Rate)
        md_lines.append("## Transaction Rate / Throughput (txn/s)\n")
        md_header = f"| Transaction | {base_display} (txn/s) |"
        md_sep = "| :--- | :---: |"
        for sys in other_systems:
            sys_name = system_display_names.get(sys, sys.capitalize())
        ...
        md_header_time = f"| Transaction | {base_display} (ms) |"
        md_sep_time = "| :--- | :---: |"
        for sys in other_systems:
            sys_name = system_display_names.get(sys, sys.capitalize())
        ...
        for txn in all_txns:
            ...
            base_time = stats[base_sys].get(txn, {}).get('time')
...
def main():
    parser = argparse.ArgumentParser(description='Parse application layer benchmark results and output performance comparisons.')
    ...
    parser.add_argument('--results-dir', default='results/app_layer', help='Base results directory for app_layer')
    parser.add_argument('--output-dir', default='reports/app_layer', help='Output directory for generated reports')
    ...
    system_display_names = {
        ...
        'adco': 'ADCo',
        'aco': 'ACo',
        'py-dbridge': 'Py-DBridge',
        'pydbridge': 'Py-DBridge'
    }
